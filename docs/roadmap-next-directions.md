...
Executed via a gated sequential workflow, then partitioned-idx finished separately: **27/27 milestones landed, suite 378 -> 515 tests, main always green**. partitioned-idx shipped as a two-level index in zrocks's own clean format (NOT RocksDB-byte-exact; default index_type=single_level unchanged). Partitioned FILTER left as a documented TODO. Earlier deferral resolved.
