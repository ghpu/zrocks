...
last_completed: 2026-05-30 TODO-audit + robustness pass. (1) TODO-reconciliation: stale format TODOs (bloom/filter_block/table_builder "full filter out of scope", compaction "obsolete .sst deletion TODO", version_edit/set kCompactPointer/CF-name) reworded to match the shipped code; honest perf/feature TODOs relabeled FUTURE(perf|feature); no remaining TODO implies a byte-exact format gap. Byte-exactness RE-PROVEN: external librocksdb v11.4.0 verify_open on a fresh flushed zrocks DB -> OPEN_OK count=11. (2) C2 real DB-level advisory LOCK file (RealEnv flock; acquired by writable DB.open + CfDB.open, not by read-only/openCfShared). (3) C3 dropColumnFamily now deletes the CF's on-disk subdir (new Env.deleteTree). (4) C1 table_cache is now a reference-counted, LRU-bounded SST handle cache (capacity = options.max_open_files; <0 = unbounded). LevelDB refcount semantics mirrored from util/cache.zig into a typed {Table,file} entry; acquire/release Handle API; newIterator stores the handle in TableIterAdapter and releases it on iterator deinit (so all LONG-lived iterator paths are correct with no caller changes); evict() now ERASE-with-pin-survival (hardens reclaimObsoleteFiles). Resident set may exceed cap while many handles are pinned (soft target, matches RocksDB). (5) Phase D: checkpoint now HARD-LINKS immutable SSTs (new Env.linkFile; RealEnv std Dir.hardLink, MemEnv content copy) with a byte-copy fallback; MANIFEST/WAL/CURRENT stay copied (matches RocksDB). (6) Phase D: FilterMergeOperand (CompactionFilter optional operand hook, applied per-operand at every level) + filter on the merge-DERIVED value (BOTTOM-LEVEL ONLY — avoids re-applying across staged partial merges) + per-compaction CompactionFilterFactory (options.compaction_filter_factory; fresh filter per compaction, owned+deinit'd by the compaction; precedence over the single filter).
...
- DB LOCK (C2, 2026-05-30): a writable DB takes a NON-BLOCKING exclusive flock on `<dir>/LOCK` (RealEnv retains the open fd in a per-path LockNode list until unlockFile). Acquired by writable DB.open (after makeDir) and once by CfDB.open on `<dbroot>/LOCK`; NOT by read-only opens or per-CF openCfShared sub-LSMs (they share the dir). Conflicting acquire -> error.IoError immediately. RealEnv.init now takes a gpa for this. src/env/real_env.zig, src/db/db.zig, src/rocks/column_family.zig.
- TABLE CACHE (C1, 2026-05-30): refcounted LRU-bounded SST handle cache (capacity = options.max_open_files; negative = unbounded). Refcount/LRU/eviction mirrored from src/util/cache.zig (LevelDB semantics: in-cache ref + per-handle ref; evictable only when unpinned; soft cap — pinned entries can exceed it). Handle API (acquire/release/get/newIterator); the iterator adapter holds the handle and releases on deinit, so LONG-lived merging-iterator paths need no caller changes; evict() = erase-with-pin-survival. src/version/table_cache.zig.
- Env.deleteTree (C3, 2026-05-30): new Env capability (RealEnv -> std Dir.deleteTree, no-op on absent; MemEnv -> prefix sweep). dropColumnFamily now deletes `<dbroot>/<cfname>/` via cfDir() (same path addCf created) after closing the sub-LSM, so a recreated-name CF no longer re-recovers stale SSTs.
